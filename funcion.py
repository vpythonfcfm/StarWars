...
class objeto():
    def __init__(self,malla,color=color.red):
        self.tris=[]
        ...
        self.init_pos(vector(malla.get_mass_properties()[1][0],malla.get_mass_properties()[1][1],malla.get_mass_properties()[1][2]),color)



    def init_pos(self,v,color):
        ...
        for n in range(len(self.v1)):
            ...
            a = vertex(pos=vec(self.v0[n][0] - self.cdg.x, self.v0[n][1] - self.cdg.y, self.v0[n][2] - self.cdg.z),color=color, normal=normalActual)
            b = vertex(pos=vec(self.v1[n][0] - self.cdg.x, self.v1[n][1] - self.cdg.y, self.v1[n][2] - self.cdg.z),color=color, normal=normalActual)
            c = vertex(pos=vec(self.v2[n][0] - self.cdg.x, self.v2[n][1] - self.cdg.y, self.v2[n][2] - self.cdg.z),color=color, normal=normalActual)
